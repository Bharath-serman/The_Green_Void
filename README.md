# The Green Void

**The Green Void** is a small **3D narrative-driven horror game** currently in development using **Unity 2022.3.41 LTS**.  
The project focuses on atmosphere, psychological tension, and minimal storytelling rather than combat-heavy mechanics.

---

## Game Overview

- **Engine:** Unity 2022.3.41 LTS  
- **Genre:** Psychological Horror / Exploration  
- **Platform:** PC (initially)  
- **Status:** In Development  

The game follows a fragmented narrative where the player explores an abandoned environment and slowly uncovers the truth behind a mysterious phenomenon known as **The Green Void**.

---

## Core Themes

- Psychological horror  
- Isolation and uncertainty  
- Environmental storytelling  
- Reality distortion and hallucinations  

## Development Focus

- Minimal UI, maximum immersion  
- Use of Unity Timeline for cutscenes and dialogues  
- Lighting, sound design, and pacing as core horror tools  
- Optimized for low-to-mid range systems  

---

## Project Status

This project is:
- A **learning + portfolio project**
- Actively evolving
- Subject to changes in story, structure, and mechanics

## License

This project is currently **not licensed for commercial use**.  
All assets and code are for educational and portfolio purposes unless stated otherwise.

---

## Author

**Bharath serman R**  
(Unity Game Developer)

---

## Screenshots
<img width="1919" height="823" alt="image" src="https://github.com/user-attachments/assets/28a50f9f-3945-43de-ad69-7ce06eb198e3" />

<img width="1920" height="822" alt="image" src="https://github.com/user-attachments/assets/35397709-a011-4f1f-b9ca-dfc61eb3b70e" />

<img width="1919" height="818" alt="image" src="https://github.com/user-attachments/assets/b7d798a6-0021-46f4-adff-16f5260b4306" />

<img width="1920" height="822" alt="image" src="https://github.com/user-attachments/assets/082ec68c-fc26-41fc-9a91-7c8e508a210c" />




> *“The Green Void is not just a place — it’s a realization.”*
